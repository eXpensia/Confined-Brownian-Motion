...
The mobility (see Eq.~(\ref{Eq.mu})) of the displacement perpendicular to the wall  thus scale as $\gamma_0^{-1} h/a$, with $\gamma_0^{-1}$ the bulk mobility. Therefore, the diffusion coefficient of a confined colloid near a wall in the lubrication approximation is hindered and inversely proportional to the particle-wall distance. It is possible to same scaling for the parrallel mobility by supposing that the particle is moving along the $x$-axis. In that case, one can find that the viscous stress is greater than the pressure and finally find that along the $x$-axis, the parallel mobility follow the same scaling as in bulk and remain constant.
...
A key observation is that due to the spatially-varying and direction-varying mobilities of the particle, the magnitude $\sqrt{2D_i(z)}$ of the Langevin forces, depend on the random position and direction of the particle. This is an example of a so-called multiplicative noise and will have some interesting consequences on the dynamical properties of the random motion. To illustrate the effect of the multiplicative noise, one can integrate Eq.~(\ref{Eq:langevin_z}) over a time $\tau$ in the absence of the external force, which leads to:
...
where $\Delta x_i$ is the spatial increment in the $i$ direction. However, the noise term the time (and consequently the position) at which the magnitude $\sqrt{2D_i(z)}$ of the Langevin force is evaluated in the integral of Eq.~(\ref{int}) needs to be specified. We write $D(z)$ as $D(z + \alpha \Delta x_z)$, with $\alpha \in [0,1]$. The value of $\alpha$ determines at which time in the interval   $[t_0, t_0 + \tau]$ the local diffusion is evaluated. However, $\alpha$ should be taken to have the long-time thermal equilibrium to be consistent with the Boltzmann distribution and. Taking into account $\alpha$, the Eq.\ref{int} in the $z$ direction becomes:

\begin{equation}
	\Delta z  = \sqrt{2D_\bot (z + \alpha \Delta z)} \textnormal{d} B_t ~,
	\label{int2}
\end{equation}

where we write $\Delta x_z = \Delta z$ for simplicity, By Taylor-expanding $D_ \bot$ at the first order in $\Delta z /z$, one has:

\begin{equation}
	D_\bot (z + \alpha \Delta z) \simeq D_\bot (z)  +  \alpha \frac{\textnormal{d} D_\bot(z)}{\textnormal{d}z} \Delta z ~.
\end{equation}

Substituting the latter in Eq.~(\ref{int2}) leads to:

\begin{equation}
	\begin{aligned}
		\Delta z &\simeq \sqrt{2D_\bot (z)  + 2\alpha \frac{\textnormal{d} D_\bot(z)}{\textnormal{d}z} \Delta z} \textnormal{d} B_t  \\
...
\begin{equation}
	\left| \alpha \frac{\textnormal{d} D_\bot(z)}{\textnormal{d}z} \frac{\Delta z }{D_\bot(z)} \right| \ll 1 ~,
\end{equation}

one Taylor-expands at first order the right-hand side of Eq.~(\ref{int3}) as:
...
By inserting Eq.~(\ref{int5}) into Eq.~(\ref{int3}), and by using the lowest order of $\Delta z$ to evaluate the first-order term, we obtain:

\begin{equation}
	\begin{aligned}
			\Delta z & \simeq \sqrt{2D_\bot (z)} \left[1 + \frac{\alpha }{2} \frac{\textnormal{d} D_\bot(z)}{\textnormal{d}z} \frac{\sqrt{2D(z)} \textnormal{d}B_t }{D_\bot(z)}\right]  \textnormal{d} B_t \\
		& =  \sqrt{2D_\bot (z)} \textnormal{d}B_t + \alpha  \frac{\textnormal{d} D_\bot(z)}{\textnormal{d}z} [\textnormal{d} B_t ]^2
	\end{aligned}
	\label{int4}
\end{equation}

Taking the average value of Eq.~(\ref{int4}) by discretizing over a time $\tau \ll 1$ such that  $\langle [\textnormal{d}B_t] ^2 \rangle_t = \tau$, gives:

\begin{equation}
	\langle \Delta z \rangle _t = \alpha  \frac{\textnormal{d} D_\bot(z)}{\textnormal{d}z} \tau = v_\mathrm{noise} \tau, 
\end{equation}

where $v_\mathrm{noise}$ represents a noise-induced spurious drift velocity. By putting back the interactions with the wall, the full expression of Eq.~\ref{int4} reads\cite{ikeda_stochastic_2014}: 

\begin{equation}
	\begin{aligned}
		\Delta z &=   \alpha  \frac{\textnormal{d} D_\bot(z)}{\textnormal{d}z}  \tau - \frac{1}{\gamma(z)}\frac{\textnormal{d} U(z)}{\textnormal{d} z}  \tau + \sqrt{2D_\bot (z)} \textnormal{d}B_t \\
		&=  \left( - \frac{1}{\gamma(z)}\frac{\textnormal{d} U(z)}{\textnormal{d} z} +  \alpha  \frac{\textnormal{d} D_\bot(z)}{\textnormal{d}z} \right) \tau + \sqrt{2D_\bot (z)} \textnormal{d}B_t \\
...
is the total drift velocity of the particle. The first term $v_\textnormal{d}$ is the drift velocity due to deterministic forces (electrostatic and gravitational interactions in our case). The spurious drift velocity $v_\mathrm{noise}$ disappears when the diffusion coefficient is homogeneous,and would also disappear along the $x$- and $y$-axes since the diffusion coefficients only depend on the colloid-wall distance $z$ (a derivative with respect to $x$ would replace the one in $z$ in the quivalent of Eq.~(\ref{Eq.total_drifts}) for the $x$ direction). Theoretically, $\alpha$ can take any value between 0 and 1. However, three canonical values are usually employed \cite{volpe_effective_2016}:  $\alpha = 0$, in the Itô convention, corresponding to the use of the initial value of $D(z)$; $\alpha = 1/2$, in the Stratonovich convention, corresponding to the mid-point value; and $\alpha = 1$  in the anti-Itô or isothermal convention, corresponding to the use of the final value. Using  Eqs.~(\ref{Eq.hindered})~(\ref{Eq:etaz_pade})~and~(\ref{Eq.total_drifts}), the deterministic drift velocity reads:
...
Finally, one can decompose the Langevin equation for a particle near a wall in two, one equation describing the motion perpendicular (\textit{i.e.} along the $z$-axis):

\begin{equation}
	V_t^z \textnormal{d}t  = \bar{v}_\mathrm{d} \textnormal{d}t + \sqrt{2D_\bot (z)}  \textnormal{d}B_t ~,
	\label{Eq.langevin_confined}
\end{equation}

and one for the parallel motion (\textit{i.e.} along the $x$- and $y$)):

\begin{equation}
	V_t^\parallel \textnormal{d}t  = \sqrt{2D_\parallel (z)}  \textnormal{d}B_t ~,
	\label{Eq.langevin_confinedx}
\end{equation}


 A typical example of drift velocity $\bar{v}$ for a colloidal particle of radius $a = 1.5 ~\mathrm{\mu m}$ in water, moving near a wall, and interacting with the latter though an electrostatic potential with a Debye length $\ell_\mathrm{D} = 50 ~ \mathrm{nm}$ and a dimensionless magnitude of the interaction $B = 4$, and evolving in a gravity field charaterized by a Boltzmann length $\ell_\mathrm{B} = 500 ~ \mathrm{nm}$, is plotted in Fig.~\ref{fig.spurious}. As one can observe, the spurious drift is not negligible.
 
\begin{figure}[ht]
	\centering
	\includegraphics{02_body/chapter3/images/spurious_drift/spurious.pdf}
	\caption{Theoritical drift velocity for a colloidal particle of radius $a = 1.5 ~\mathrm{\mu m}$ in water and at a distance $z$ from the wall. The physical parameters $\ell_\mathrm{D} = 50 ~ \mathrm{nm}$, $B = 4 ~k_\mathrm{B}T$ and $\ell_\mathrm{B} = 500 ~ \mathrm{nm}$.} 
	\label{fig.spurious}
\end{figure}

\subsubsection{Fokker-Plank equation}

The Fokker-Plank equation is an alternative way to describe Brownian motion. Instead of explicitly calculating a Brownian trajectory by solving the Langevin equation, Fokker-Plank equation describes the Probability Density Function $P(X_t, X_0; t)$ inposition, where for simplicity we place ourselve in 1D, with $X_t$ denoting the particle position at a time $t$ and $X_0$ its initial ($t=0$) position. To derive the Fokker-Plank equation, let us start by taking a generic Langevin equation in 1D:

\begin{equation}
	\textnormal{d}X_t = u(X_t) + g\textnormal{d}B_t ~,
	\label{SDE2}
\end{equation}

where $u$ is the drift due to external forces and $g$ is the magnitude of the random force. Let consider the average value of an arbitrary function $f(X_t)$ for a stochastic process obeying Eq.~(\ref{SDE2}), which started at position $x_0$ at time $t=0$. By definition, this ensemble average reads \cite{le_bellac_equilibrium_2004}:
...
\begin{equation}
	\begin{aligned}
		\left\langle \frac{\textnormal{d}f(X_t)}{\textnormal{d}t} \right\rangle_t  &\simeq \frac{1}{\textnormal{dt}}\left\langle   \frac{\partial f(X_t)}{\partial X_t} u(X_t) \textnormal{d}t + \frac{\partial f(X_t)}{\partial X_t} g(X_t) \textnormal{d}B_t + \frac{1}{2} \frac{\partial^2 f(X_t)}{\partial X_t^2} g^2 (X_t) \textnormal{d}B_t^2   \right\rangle_t \\
		& =  \frac{1}{\textnormal{dt}} \left( \frac{\partial f(X_t)}{\partial X_t} u(X_t) \textnormal{d}t + \frac{1}{2} \frac{\partial^2 f(X_t)}{\partial X_t^2} g^2 (X_t) \textnormal{d}t \right) \\
		& = \frac{\partial f(X_t)}{\partial X_t} u(X_t) + \frac{1}{2} \frac{\partial^2 f(X_t)}{\partial X_t^2} g^2 (X_t) ~.
	\end{aligned}
	\label{fokker2}
\end{equation}
...
\begin{equation}
	Gf = \frac{1}{2} g^2 (X_t) \frac{\partial ^2 f(X_t)}{\partial X_t^2} + u(X_t) \frac{\partial f(X_t)}{\partial X_t} ~.
\end{equation}
...
\begin{equation}
	\frac{\partial P(X_t, X_0 ; t)}{\partial t }= \frac{\partial ^2}{\partial X_t^2} \left[\frac{g^2 (X_t)}{2}P(X_t, X_0 ; t)\right] - \frac{\partial}{\partial X_t} \left[u(X_t) P(X_t, X_0 ; t)\right] ~.
	\label{Eq.Forward_Fokker_plank}
\end{equation}

The latter is called Forward because the partial differential equation is written in terms of the variable $X_t$, \textit{i.e.} the position of the particle, at time $t$. For a Brownian motion near a wall, using the previously determined drifts velocity $\bar{v}_\textnormal{d}$ along $z$ of Eq.~(\ref{Eq.total_drifts}), the Fokker-Plank equation for the motion along the $z$-axis, perpendicular to the wall reads:

\begin{equation}
	\frac{\partial P(z, z_0 ; t)}{\partial t} = \frac{\partial ^2}{\partial z ^2} \left[ D_\bot(z)  P(z, z_0 ; t) \right]   -  \frac{\partial }{\partial z} \left[\bar{v}_\textnormal{d} P(z, z_0 ; t)\right] ~,
	\label{fp}
\end{equation}

with $z$ the particle position along the $z$-axis. In particular, the stationary solution of Eq.~(\ref{fp}), \textit{i.e.} for $\partial p / \partial t = 0$, is given by the Gibbs-Boltzmann distribution $P_\textnormal{eq}(z) $ (see Eq.~(\ref{Eq.Peq})). The solution of Eq.~(\ref{fokker3}) with the particle starting at position $z_0$, at time $t = 0$ reads:

\begin{equation}
	\begin{aligned}
		P(z, z_0; t) &= \exp\{G^\dagger t\} P(z, z_0; 0) \\
		& = \exp \left\{ \frac{\partial ^2}{\partial z ^2}  D_\bot(z)  t   -  \frac{\partial }{\partial z} \bar{v}_\textnormal{d} t\right\} \delta (z - z_0) ~.
	\end{aligned}
	\label{Eq.solfokker}
\end{equation} 

\subsubsection{Fokker-Plank and multiplicative noise}

Due to the hindered diffusion coefficient the magnitude of the random force $g$ now depends on the particle position $X_t$. Therefore, for a displacement between a time $t$ and $t+\tau$, the integration of the noise term $\int_t ^{t+\tau}g(X_t)\mathrm{d}B_t$ is not trivial and the time at which the random force magnitude $g(X_t)$ is evaluated needs to be answered. In this part we derive the Fokker-Plank equation when the system is subjected to multiplicative noise. Let us write a generic Langevin equation for a near-wall confined particle:

\begin{equation}
	\mathrm{d}X_t = u(X_t) + g(X_t)\mathrm{d}B_t~.
	\label{sde.multipl}
\end{equation}
...
The expension of first term gives $u(X_t)\tau$. However, for the second term, the position at which $g(X_t)$ is evaluated needs to be addressed. The second integral of Eq.~{\ref{sde.multipl2}} is not unequivocally defined as \cite{sancho_brownian_2011}:
...
which is au Gaussian process with zero mean and variance $\langle \Delta W(t)^2 \rangle = \Delta t $. The parameter $\alpha$ in Eq.~{\ref{sde.alpha}} corresponds to stochastic interpretation of the Langevin equation, and at which point in the interval $[t, t+\tau]$ the Lanvin force magnitude $g$ is evaluated. Theoretically, $\alpha$ can take any value between 0 and 1. However, two canonical values are usually employed :  $\alpha = 0$, in the Itô convention \cite{ito_stochastic_1944}, corresponding to the use of the initial value of $g(X_t))$; $\alpha = 1/2$, in the Stratonovich convention \cite{stratonovich_new_1966}, corresponding to the mid-point value $g(X_{t + 1/2\tau}))$. 

To derive the Fokker-Plank equation we now need to expand Eq.~(\ref{sde.multipl2}) to first order in $\tau $. Combining Eqs.~(\ref{sde.multipl2})~and~(\ref{sde.alpha}), at the first order in $\tau^{1/2}$, Eq~.(\ref{sde.alpha}) can be approximated by:
...
where $g'(X_t) = \partial_{X_t} g(X_t) $. By finally combining Eqs.~(\ref{sde.multipl2})~and~(\ref{sde.taylorend}), one can expend Eq.~(\ref{sde.multipl2}) to the first order in $\tau$:

\begin{equation}
	X_{t+\tau } \simeq X_t + u(X_t) \Delta W(t) + \alpha g(X_t)g'(X_t)[\Delta W(t)]^2 ~.
\end{equation}
...
where $K_1$ and $K_2$ are the first two differential moments, and invoking the Kramers-Moyal expansion \cite{sancho_brownian_2011, stratonovich_new_1966}, $K_1$ and $K_2$ are obtained by:
...
In that case, by choosing the isothermal convention ($\alpha = 1$), Eq.~(\ref{Eq.total_drifts}) near the surface simplifies to:
...
The long-time \gls{PDF} in position, for $\alpha = 0 , 0.5 \text{ and } 1 $ are shown in Fig.~\ref{fig.pdf_vs_alpha}. We see that the isothermal convention of the noise gives the correct distribution. In the other two cases, we observe that the particle is more likely to be found closer to the surface, as a result of the missing compensating spurious drift of Eq.~(\ref{Eq.spurious_drift}). However, defining derivative in other way (\textit{i.e.} Anti-itô or Stratonovich convention) would allow to employ any $\alpha$ values. 

\begin{figure}[H]
	\centering
	\includegraphics{02_body/chapter3/images/simulation_confined_Brownian_motion/Peq_vs_alpha.pdf}
	\caption{Simulated, Long-term Probability Density Function of the height of the particle for different convention as indicated. The solid black line represents the expected Gibbs-Boltzmann distribution. The physical parameters are: $a = 1.5 ~ \mathrm{\mu m}$, $\rho_\mathrm{p} = 1050 ~\mathrm{kg.m^{-3}}$, $\ell_\mathrm{D} = 50$ nm, $B = 4$ and $\ell_\mathrm{B} = 577$ nm. }
...
Let us now analyze the experimental data obtain through the Mie tracking (see section \ref{chap:LM_fit}). In the near-wall Brownian dynamics theory presented section~\ref{sec:confined}, we considered distance $z$ between the wall and the colloidal particle surface. However, it is not the height measured by the Mie tracking, since the latter measures the distance between the objective-lens focal plane and the particle center. Therefore, we measure the trajectory up to an offset, \textit{i.e.} the ojective-lens focal plane to wall distance. To have the correct measured height, we suppose that the particle does approach very closely to the wall, such that the minimal measured distance is the focal plane to the wall distance. From that assumption, we then a moving-minimum along the trajectory to set the minimal value of the trajectory to zero. The moving minimum can be calculated using the following Python function:
...
\begin{equation}
	...
	\label{Eq:PDzshort}
\end{equation}
against the Gibbs-Boltzmann distribution. Thee latter equation can alternatively be written as an integral over the diffusion coefficient such that:
\begin{equation}
...
Wwere the \mintinline{python}{P_eq} function has been described in section \ref{Section:sphere-wall}. 
...
Let us now explain the force measurement from dirfts. By averaging the overdamped Langevin of Eq.~(\ref{Eq.langevin_confined}) over a fine-enough $z$-binning grid and a short-enough time interval $\Delta t$, one gets in the Itô convention (corresponding to our definition of $\Delta z$):
\begin{equation}
