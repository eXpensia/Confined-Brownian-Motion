...
In the bulk, a fluid is electrically neutral.Thus, the first term vanishes since $\sum_i z_i c_i^0 = 0$. Thus, one can linearize Eq.~(\ref{Eq:Poisson-boltzmann}) to write the Debye-Hünkel equation:
...
where G in an operator called the generator and is defined by its action on a function $f$ by:
...
In the latter snippet, the evaluation is done for $\Delta z$, however, to compute $P_x(\Delta x)$ one should just change the \mintinline{python}{Dz(z)} function to compute the parallel diffusion coefficient $D_\parallel$. As a \gls{PDF} $P$ should be normalized such that $\int P = 1$, I added an extra step to ensure \gls{PDF} normalization along the evaluation.
