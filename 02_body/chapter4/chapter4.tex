...
\begin{equation}
	P_\mathrm{Heq}(x) =A \exp \left[ -\frac{k_\mathrm{k_H}x^2}{2k_\mathrm{B}T} \right]~,
	\label{Peqstuck}
\end{equation}

where $A$ is a normalization constant such that $\int _0 ^\infty P_\mathrm{Heq\mathrm{d}x} =1$. The experimental Gibbs-Boltzmann equilibrium \gls{PDF} $P_\mathrm{Heq}$ is shown in Fig.~\ref{fig.pdfstuck}. We can observe a good agreement between the fitted distribution from Eq.~\ref{Peqstuck}. From the latter fit, we measure more precisely $k_\mathrm{k_\mathrm{H}} = 1.738 \pm 0.004 ~\mathrm{\mu N . m^{-1}}$.
