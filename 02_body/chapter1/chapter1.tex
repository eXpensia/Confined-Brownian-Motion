...
On the right-hand side, since $\varphi(x)$ is an even function, the second term vanishes. Taking into account Eq.\ref{Eq:ein_def_phi} and invoking the definition:
...
To have a more quantitative approach, one can compute the Mean Relative Squared Error (\gls{MRSE})) between the measured \gls{PDF} $P_c(w_i)$ and $P(w_i)$ as a function of the number of generated numbers $N$, such as:

\begin{equation}
	\textnormal{MRSE} = \left. \langle \frac{\left( P_c(w_i) -P(w_i) \right)^2  } {P(w_i) ^2}\rangle \right|_N
\end{equation}

where the notation $\left. \right|_N$ denotes that $N$ numbers are generated. Additionaly, since we measure the $P_c(w_i)$ by doing an histogram, the question of how many bins is used should be answered. It is possible to use the Freedan-Diaconis rule \cite{freedman_histogram_1981} to compute the width of the bins to be used in a histogram, this rule reads:
...
\end{minted}
 ...
 As we can see on the Fig.\ref{fig:MSEwi}, for $\tau$ varying between $10^{-2}$ and $10$ seconds, and, $N$ between $10$ and $10^6$, the \gls{MRSE} decreases as $N$ increases. More over, it is interesting to observe that the \gls{MRSE} is greater as $\tau$ increases for a fix $N$ value. Indeed, as an example we would need to only generate $N = 10^{-3}$ numbers to obain a \gls{MRSE} of $10^{-4}$ for $\tau = 0.1~\textnormal{s}$, while we would need to $N= 10^6$ for $\tau = 1 ~ \mathrm{s}$. 
...
Now that the Langevin equation is known numerically, one could use it to simulate some Brownian trajectories. A simple way to do the simulation is shown using Python in the Jupyter notebook framework in the appendix.\ref{app:simfulllangevin}. A set of trajectory simulated for a fictive particle of radius $a = 1 ~ \mathrm{\mu m}$ and mass $m = 10 ~ \mathrm{\mu g}$ in water is shown in Fig.\ref{fig:inertial_langevin}-a). For such a particle the diffusive characteristic time is $\tau_\mathrm{B} = 0.53 s$. Moreover, as one can see on the Fig.\ref{fig:inertial_langevin}-b), the MSD is correctly modeled by the Eq.\ref{Eq.shorttimemsd} for $\tau << \tau_\mathrm{B}$ and by the Eq.\ref{eq:longtimemsd} for $\tau >> \tau_\mathrm{B}$. Please note that for non-continuous data such as the simulation or experimental trajectories, and for a given time increment $\Delta t$, the \gls{MSD}, is generally defined by:

\begin{equation}
	\left. \langle \Delta x^2 \rangle \right|_t = \left. \langle ( x(t + \Delta t) - x(t)) ^2 \rangle \right|_t ~,
\end{equation}

where the average $	\left. \langle \rangle \right|_t $ is performed over a time $t$. Additionally, it can be numerically using the following Python function.
...
\begin{minted}
	...
	{python}
def msd(x, Dt):
	"""Function that return the MSD for a list of time index Dt for a trajectory x"""
	_msd = lambda x, t: np.mean((x[:-Dt] - x[Dt:]) ** 2)
...
\begin{figure}[h]
	...
	\caption{a) Set of $100$ trajectories simulated using the full Langevin equation for particle of a radius  $a = 1 ~\mathrm{\mu m}$ and a mass $m = 10 ~\mathrm{\mu g}$ in water, $\eta = 0.001 ~\mathrm{Pa.s}$. The simulations are done with a time step $\tau = 0.01~ \mathrm{s}$. b) Bullets represents the measured Mean Squared Displacement (\gls{MSD}) of the simulated trajectories. The plain black line represents the characteristic time of the diffusion, here $\tau_\mathrm{B} = m / \gamma = 0.53 ~\mathrm{s}$. The dotted line represents the MSD theory when $t << \tau _\mathrm{B}$, $\mathrm{MSD} = \tau^2 k_\mathrm{B}T / m$. The dashed line when $t >> \tau _\mathrm{B}$, $\mathrm{MSD} = 2D\tau$. A detailed explanation of the simulation process can be found in the appendix.\ref{app:simfulllangevin}.  }
	\label{fig:inertial_langevin}
\end{figure}

Additionally, as we have seen earlier, the Langevin Equation can be simplified to it's over-damped version Eq.\ref{Eq.overdamped_SDE}. In this case, the time step of the simulation  $\tau$ should be greater than the diffusion time $\tau_\mathrm{B}$. Thus, one who is interested at the long time statistical properties of the Brownian motion can use the over-damped Langevin equation. In this case by putting $m=0$ into Eq.\ref{Eq.numfulllangevin}, one can write $x_i$ as:
...
The statistical properties at long time could be retrieved by simulating Brownian motion using the full Langevin equation. But, since the integration scheme used for Eq.\ref{Eq.numfulllangevin} requires  $\tau << \tau_\mathrm{B}$  long simulation are necessary to retrieve the over-damped properties. As an example, for the Fig.\ref{fig:bulkbrown} I directly used Eq.\ref{Eq.shortnumlangevin} for the simulation using the Python Snippet below.
