...
where IQR is the interquartil range, and $\{w_i\}$ a sample of $N$ random numbers. Moreover, one should only take $2$ bins as a minimum. The actual number of bins can be computed using the following Python snippet.
...
def iqr(wi):
	"""Function to compute interquartil range."""
...
\end{minted}
 ...
 As we can see on the the Fig.\ref{fig:MSEwi}, for $\tau$ varying between $10^{-2}$ and $10$ seconds, and, $N$ between $10$ and $10^6$, the \gls{MRSE} decreases as $N$ increseases. More over, it interesting to observe that the \gls{MRSE} is greater as $\tau$ increases for a fix $N$ value. Indeed, as an exemple we would need to only generate $N = 10^{-3}$ numbers to obain a \gls{MRSE} of $10^{-4}$ for $\tau = 0.1~\textnormal{s}$, while we would need to $N= 10^6$ for $\tau = 1 ~ \mathrm{s}$. 
